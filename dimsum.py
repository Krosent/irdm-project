...
class Dimsum:
    ...
    def map(self, row):
        ...
        if row.count_nonzero() >= 1:
            ...
            for ai in pairs:  # iterate over all the pairs
                ...
                cj = first_index
                ck = second_index
                cj_normalized = dimsum.norms[cj]  # list starts from 0
                ck_normalized = dimsum.norms[ck]
    ...
    def reduce(self, mapper_results):
        _dict = dimsum.prepare_reduce_input(mapper_results)

        for key, values in _dict.items():
            rating_indexes = key.split("-")

            cj_normalized = dimsum.norms[int(rating_indexes[0])]
            ck_normalized = dimsum.norms[int(rating_indexes[1])]
